// Sistema de Autenticação CORRIGIDO - Usando UUID do Supabase
...
export const register = async (name, email, password) => {
  ...
  try {
    ...
    // 3. INSERIR usuário no Supabase SEM especificar ID (usa UUID automático)
    const { data: newUser, error: insertError } = await supabase
      .from('appmari_users')
      .insert([{
        // NÃO especificar ID - deixa o Supabase gerar UUID automaticamente
        name: name.trim(),
